...
import { ModalProps } from "../../Types/typings";
...
export function Modal({ todo, onDeletedTask, onCompletedTodo }: ModalProps) {
  ...
  const completedDateFormatted =
    todo.done &&
    format(new Date(todo.completedDate), "dd'/'LL'/'yyyy' às 'HH':'mm'h'", {
      locale: ptBR,
    });
  ...
  return (
    <>
      <button onClick={openModal} className={styles.openModal} title="Ver mais">
        <Info size={20} weight="bold" color="#f2f2f2" />
      </button>

      <Dialog open={isOpen} onClose={closeModal} className={styles.modal}>
        <Dialog.Panel className={styles.modalContainer}>
          <button
